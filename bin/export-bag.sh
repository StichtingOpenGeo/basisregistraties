...
echo "CREATE INDEX adres_postcode ON adres (postcode varchar_pattern_ops);" | psql -U postgres bag

echo "copy (select distinct substring(postcode from 1 for 4) as pp4 from adres order by pp4) to '/tmp/bag_csv/postcodes4pp.csv';" | psql -U postgres bag
