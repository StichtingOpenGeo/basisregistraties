...
echo "CREATE INDEX adres_postcode ON bagactueel.adres (postcode varchar_pattern_ops);" | sudo -u postgres psql

echo "copy (select distinct substring(postcode from 1 for 4) as pp4 from bagactueel.adres order by pp4) to '/tmp/bag_csv/postcodes4pp.csv';" | sudo -u postgres psql
